...
class Employee {
    ...
    public function getAll($page = 1, $limit = 10, $search = '', $department = '', $status = '') {
        ...
        if($search) {
            $countQuery .= " AND (e.name LIKE ? OR e.employee_code LIKE ? OR e.email LIKE ?)";
        }
    ...
    public function create($data) {
        ...
        try {
            ...
            $profileStmt->execute([$userId, $data['name'], $data['phone_number']]);
    ...
    public function validate($data) {
        ...
        $requiredFields = ['name', 'gender', 'birth_date', 'phone', 'email', 'address', 'department_id', 'position_id'];
