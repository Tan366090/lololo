...
function getPayrolls($conn) {
    try {
        ...
        $formattedPayrolls = array_map(function($payroll) {
            return [
                ...
                'payment' => [
                    ...
                    'reference' => $payroll['payment_reference'] ?? 'PAY-' . $payroll['payroll_id'] . '-' . date('Ymd', strtotime($payroll['generated_at']))
                ],
                'status' => [
                    'code' => $payroll['status'],
        ...
        // Output single JSON response and exit
...
function getPayrollById($conn, $id) {
    ...
    $query = "SELECT p.*, e.employee_code, e.name as employee_name, 
                     d.name as department_name
              FROM payroll p
              LEFT JOIN employees e ON p.employee_id = e.id
              LEFT JOIN departments d ON e.department_id = d.id
...
function exportDepartmentPayrollReport($conn, $departmentId, $month, $year) {
    try {
        ...
        $query = "SELECT p.*, e.code as employee_code, e.name as employee_name,
...
class PayrollAPI {
    ...
    public function getPayrolls() {
        try {
            // Check rate limit
            checkRateLimit(getCurrentUserId());
            ...
            // Check cache
            $cacheKey = "payrolls_" . md5(json_encode($_GET));
            ...
            if (!empty($search)) {
                $query .= " AND (e.employee_code LIKE ? OR e.name LIKE ?)";
                $params[] = "%$search%";
                $params[] = "%$search%";
            }
            ...
            $formattedPayrolls = array_map(function($payroll) {
                return [
                    ...
                    'salary' => [
                        'base' => formatMoney($payroll['base_salary_period']),
                        'allowances' => formatMoney($payroll['allowances_total']),
                        'bonuses' => formatMoney($payroll['bonuses_total']),
                        'deductions' => formatMoney($payroll['deductions_total']),
                        'gross' => formatMoney($payroll['gross_salary']),
                        'tax' => formatMoney($payroll['tax_deduction']),
                        'insurance' => formatMoney($payroll['insurance_deduction']),
                        'net' => formatMoney($payroll['net_salary'])
                    ],
                    'payment' => [
                        'date' => $payroll['payment_date'],
                        'method' => $payroll['payment_method'],
                    ...
                    'created_by' => [
                        'id' => $payroll['generated_by_user_id'],
                        'username' => $payroll['generated_by'],
                        'email' => $payroll['created_by_email']
                    ],
                    'created_at' => $payroll['generated_at'],
                    'notes' => $payroll['notes']
            ...
            // Get details for each payroll
            foreach ($formattedPayrolls as &$payroll) {
                // Get allowances
                $stmt = $this->conn->prepare("
                    SELECT a.name, pa.amount 
                    FROM payroll_allowances pa
                    JOIN allowances a ON pa.allowance_id = a.allowance_id
                    WHERE pa.payroll_id = ?
                ");
                $stmt->execute([$payroll['id']]);
                $payroll['details']['allowances'] = $stmt->fetchAll(PDO::FETCH_ASSOC);

                // Get bonuses
                $stmt = $this->conn->prepare("
                    SELECT b.bonus_type, pb.amount 
                    FROM payroll_bonuses pb
                    JOIN bonuses b ON pb.bonus_id = b.bonus_id
                    WHERE pb.payroll_id = ?
                ");
                $stmt->execute([$payroll['id']]);
                $payroll['details']['bonuses'] = $stmt->fetchAll(PDO::FETCH_ASSOC);

                // Get deductions
                $stmt = $this->conn->prepare("
                    SELECT d.name, pd.amount 
                    FROM payroll_deductions pd
                    JOIN deductions d ON pd.deduction_id = d.deduction_id
                    WHERE pd.payroll_id = ?
                ");
                $stmt->execute([$payroll['id']]);
                $payroll['details']['deductions'] = $stmt->fetchAll(PDO::FETCH_ASSOC);

                // Get approvals
                $stmt = $this->conn->prepare("
                    SELECT pa.*, u.username as approver_name
                    FROM payroll_approvals pa
                    JOIN users u ON pa.approver_id = u.user_id
                    WHERE pa.payroll_id = ?
                    ORDER BY pa.approval_level ASC
                ");
                $stmt->execute([$payroll['id']]);
                $payroll['approvals'] = $stmt->fetchAll(PDO::FETCH_ASSOC);
            }
            ...
            // Log activity
            logActivity(getCurrentUserId(), 'GET_PAYROLLS', "Retrieved payroll list");
    ...
    public function getById($id) {
        try {
            ...
            if ($stmt->rowCount() === 0) {
                ...
                echo json_encode([
                    'success' => false,
                    'message' => 'Không tìm thấy thông tin lương'
                ]);
                return;
            }
            ...
            ");
            $stmt->execute([$id]);
            $formattedPayroll['details']['allowances'] = $stmt->fetchAll(PDO::FETCH_ASSOC);
            ...
            ");
            $stmt->execute([$id]);
            $formattedPayroll['details']['bonuses'] = $stmt->fetchAll(PDO::FETCH_ASSOC);
            ...
            ");
            $stmt->execute([$id]);
            $formattedPayroll['details']['deductions'] = $stmt->fetchAll(PDO::FETCH_ASSOC);
            ...
            ");
            $stmt->execute([$id]);
            $formattedPayroll['approvals'] = $stmt->fetchAll(PDO::FETCH_ASSOC);

            // Payments
            $stmt = $this->conn->prepare("
                SELECT pp.*, u.username as processed_by
                FROM payroll_payments pp
                LEFT JOIN users u ON pp.processed_by_user_id = u.user_id
                WHERE pp.payroll_id = ?
            ");
            $stmt->execute([$id]);
            $formattedPayroll['payments'] = $stmt->fetchAll(PDO::FETCH_ASSOC);
            ...
            // Log activity
            logActivity(getCurrentUserId(), 'GET_PAYROLL', "Retrieved payroll details for ID: $id");
